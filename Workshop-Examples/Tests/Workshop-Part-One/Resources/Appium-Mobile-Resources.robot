...
Set Up Chrome In Android
    [Arguments]     ${APPIUM_REMOTE_URL}    ${DEVICE_NAME_ANDROID}     ${APPIUM_PORT}
    Open Application    ${APPIUM_REMOTE_URL}    platformName=${PLATFORM_NAME_ANDROID}    deviceName=${DEVICE_NAME_ANDROID}    PORT=${APPIUM_PORT}    automationName=${ANDROID_AUTOMATION_NAME}    browserName=Chrome    newCommandTimeout=3600
    Go To Url    %{APP_URL}
    Sleep    1s

Set Up Safari In IOS
    [Arguments]     ${APPIUM_REMOTE_URL}    ${DEVICE_NAME_IOS}    ${APPIUM_PORT}    ${APPIUM_WDALOCALPORT}
    Open Application    ${APPIUM_REMOTE_URL}    platformName=${PLATFORM_NAME_IOS}    platformVersion=${PLATFORM_VERSION_IOS}    devicetype=simulator
    ...    deviceName=${DEVICE_NAME_IOS}    PORT=${APPIUM_PORT}    WDALOCALPORT=${APPIUM_WDALOCALPORT}    automationName=${IOS_AUTOMATION_NAME}     startIWDP=true
